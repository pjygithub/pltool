...
if (isset($_COOKIE['lang']) and $_COOKIE['lang'] == 'en') {
    include(dirname(__DIR__) . '/language/lang.en.php'); //英文语言文件
} else {
    include(dirname(__DIR__) . '/language/lang.zh_CN.php'); //中文语言文件
}
...
<script>
    ...
    // waterprint = '<?php echo 'machine_ #' . $_COOKIE['machine'] . ' - func_ ' . $_COOKIE['func'] . ' | dir_ ' . $dirname . ' - type_' . $_COOKIE['type'] . ' - model_' . $_COOKIE['model']; ?>_' + getRnd(1000, 9999);
    // waterprint1 = '<?php echo 'show_' . $_COOKIE['show'] . ' - lang_' . $_COOKIE['lang'] . ' - ' . date('Y/m/d', $_COOKIE['date_start']) . ' - ' . date('Y/m/d', $_COOKIE['date_end']); ?>';
    waterprint = '<?php echo '机台号#' . $_COOKIE['machine'] . ' - 查询类型1_' . $_COOKIE['func'] . ' | 资源目录_' . $dirname . ' - 查询类型2_' . $_COOKIE['type'] . ' - 模块_' . $_COOKIE['model']; ?> - 随机码' + getRnd(1000, 9999);
    waterprint1 = '<?php echo '显示方式_' . $_COOKIE['show'] . ' - 语言_' . $_COOKIE['lang'] . ' - 选择日期' . date('Y/m/d', $_COOKIE['date_start']) . ' - ' . date('Y/m/d', $_COOKIE['date_end']); ?>' + ' - 从登录账户：' + '<?php echo $username ?>';
    ...
    if (timeData.length >= 600) {
        var startz = 100 - (100 / timeData.length) * 720;
        startz = parseInt(startz, 10) * 0;
    } else {
        var startz = 0;
    }
        ...
        tooltip: {
            ...
            formatter: function(params, ticket, callback) {
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    htmlStr += '<span>' + seriesName + '</span></td><td style="text-align:right;padding-left:2em;">';
