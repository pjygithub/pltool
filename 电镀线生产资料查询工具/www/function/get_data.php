    ...
    for ($i = 0; $i < $count_Dates; $i++) {
        ...
        if (isset($contents) and strlen($contents) > 0 and $contents != "" and $contents != null) {
            ...
            for ($j = 0; $j < $count_lines; $j++) {
                ...
                // 档案名称输入包含空格导致识别错误
                $re = '/[0-9a-zA-Z]{4,5}[-][0-9a-zA-Z]{13,18}[( （][a-zA-Z0-9]{0,}[,][a-zA-Z0-9]{0,}[)）].txt/mgi';
                $str = $lines_j;
                preg_match_all($re, $str, $matches, PREG_SET_ORDER, 0);
                // Print the entire match result
                $count_mact = count($matches);
                for ($m = 0; $m < $count_mact; $m++) {
                    $tem_ = str_replace(',', ' ', $matches[$m][0]);
                ...
                // Print the entire match result
                $count_mact = count($matches);
                for ($n = 0; $n < $count_mact; $n++) {
                    $tem_ = str_replace(',', '&', $matches[$n][0]);
                    $lines_j = str_replace($matches[$n][0], $tem_, $lines_j);
                }
