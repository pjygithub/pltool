...
if (!isset($_GET['tbheaderIDID'])) {
    $_GET['tbheaderIDID'] = "";
} else {
    $tbheaderIDID = urldecode($_GET['tbheaderIDID']);
}
...
// 执行SQL语句
$results_lang = $db_handle->query('SELECT * FROM `tb_lang_zh_cn` where `en_source`="' . $tbheaderIDID . '"  ORDER BY `type` ASC, `en_source` asc');
...
<body>
    ...
    <form action="./function/lang_zh_cn_only_func.php?tbheaderIDID=<?php echo $_GET['tbheaderIDID'] ?>" method="post">
        ...
        <table class="layui-table" lay-even>
            ...
            <tbody id="tb_data">
                ...
                echo '<tr>';
                echo '<td>' . ($i + 1) . '</td>';
                echo '<td><input type="text" name="tbheaderIDID" value="' . $_GET['tbheaderIDID'] . '" readonly unselectable="on" style="background-color: #e2e2e2;border-bottom: unset;"></td>';
                echo '<td><input type="text" name="zh_cn_only" value="' . $arr_zh_cn[0]['zh_cn'] . '"></td>';
                ?>
            </tbody>
        </table>
    </form>
